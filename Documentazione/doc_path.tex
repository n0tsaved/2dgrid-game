...
\end{figure}
\par{A fine ricerca viene memorizzato il costo del percorso trovato nel mapping $pathcost(1)$. A questo punto l'agent sorgente si muove lungo il percorso da (0,1) a (1,2).
L'agent  target si muove da (7,2) a (8,3), uscendo fuori dal percorso che l'agent sorgente aveva calcolato.
Quest'ultimo si vedr\`a dunque costretto a pianificare un nuovo percorso. La seconda iterazione \`e mostrata in figura \ref{fig:AA2}.} 
\begin{figure}[H]
\centering
\begin{tikzpicture}
...
\centering
\includegraphics[scale=0.30]{/home/notsaved/theta2.png}
...
\begin{subfigure}{.5\textwidth}
  \centering
  \includegraphics[width=.8\linewidth]{/home/notsaved/theta2.png}
  \caption{TrailMax su mappa outdoor}
  \label{fig:sfig1}
\end{subfigure}%
\begin{subfigure}{.5\textwidth}
  \centering
  \includegraphics[width=.8\linewidth]{/home/notsaved/theta3.png}
...
\par{
Il motivo per il quale LMTAA*, pur espandendo meno nodi degli altri algoritmi di ricerca, non risulta il migliore anche in termini temporali di A* \`e perch\`e LMTAA* esegue un numero maggiore di operazioni basilari per una singola iterazione rispetto ad A*, quali ad esempio accessi ad \emph{hashmap} di dimensioni molto grandi. Queste operazioni di accesso ad hashmap giustificano l'\emph{overhead} generato da una singola iterazione di LMTAA* rispetto ad A*.
}
\par{
