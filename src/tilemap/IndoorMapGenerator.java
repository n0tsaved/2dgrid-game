...
public class IndoorMapGenerator extends MapGenerator {
    ...
    public void divide(Tile[][] grid,int x, int y,int width, int height,boolean orientation){
        if (width< 12||height< 12 || width*height<Math.pow(ROOM_MIN_SIZE,2)) return;
        boolean horizontal =orientation ==true;

        int wx =x +(horizontal ? 0: r.nextInt(((width / 4) * 3) - ((width / 4) + 1)) + (width / 4));
        int wy =y +(horizontal ? r.nextInt(((height / 4) * 3) - ((height / 4) + 1)) + (height / 4):0);

        int px =wx +(horizontal ? r.nextInt(width/2) :0);
        int py =wy +(horizontal ?0: r.nextInt(height/2));
